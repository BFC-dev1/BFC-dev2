...
ARCHIVO: index.php / asistencia_entrenadores.php
=========================================================
*/
...
$modulo_actual = 'Asistencia Entrenadores';
$submodulo_actual = '';
$menu_modulo = [];
...
<div class="tabla-usuarios-contenedor">
    <table class="table table-bordered table-hover text-center align-middle tabla-usuarios">
        ...
        <tbody>
        <?php
        // Consultar usuarios activos con rol 'entrenador'
        ...
        <tr id="fila_<?= $row['id'] ?>">
            ...
            <td data-label="Hora Entrada">
                <input type="time" class="form-control text-center mx-auto" id="entrada_<?= $row['id'] ?>" style="max-width: 140px;">
            </td>

            <td data-label="Hora Salida">
                <input type="time" class="form-control text-center mx-auto" id="salida_<?= $row['id'] ?>" style="max-width: 140px;">
            </td>

            <td data-label="Total Horas">
                <span id="total_<?= $row['id'] ?>" class="badge bg-light text-dark fs-6 border">0.00 hrs</span>
            </td>

            <td data-label="Estado" class="celda-estado-usuario">
                <span id="badge_estado_<?= $row['id'] ?>" class="badge bg-secondary">Sin Registro</span>
            </td>

            <td data-label="Observaciones">
                <input type="text" class="form-control form-control-sm mx-auto" id="obs_<?= $row['id'] ?>" placeholder="Opcional..." style="max-width: 200px;">
            </td>

            <td data-label="Acciones" class="celda-acciones-usuario">
                <div class="usuario-botones">
                    <button class="btn btn-success btn-sm" title="Marcar Hora Actual" onclick="marcarAhora(<?= $row['id'] ?>)">
                        <i class="fa-solid fa-clock"></i> Marcar
                    </button>
                    <button class="btn btn-warning btn-sm text-white" title="Marcar Ausente" onclick="marcarAusente(<?= $row['id'] ?>)">
                        <i class="fa-solid fa-user-slash"></i> Ausente
                    </button>
                    <button class="btn btn-primary btn-sm" title="Guardar Cambios" onclick="guardar(<?= $row['id'] ?>)">
                        <i class="fa-solid fa-floppy-disk"></i> Guardar
                    </button>
                    <button class="btn btn-danger btn-sm" title="Eliminar Registro" onclick="limpiar(<?= $row['id'] ?>)">
...
function cargarAsistencia(){
    ...
    .then(data => {
        ...
        data.forEach(item => {
            let inputEntrada = document.getElementById(`entrada_${item.usuario_id}`);
            let inputSalida = document.getElementById(`salida_${item.usuario_id}`);
            let inputObs = document.getElementById(`obs_${item.usuario_id}`);
            let spanTotal = document.getElementById(`total_${item.usuario_id}`);
            let badgeEstado = document.getElementById(`badge_estado_${item.usuario_id}`);
...
function guardar(usuario_id, estadoForzado = null){
    ...
    .then(res => {
        if(res.status === "ok"){
            ...
            Swal.fire('Error', res.mensaje || "Error al guardar registro", 'error');
        }
    });
}
...
function limpiar(usuario_id){
    Swal.fire({
        ...
        text: "Se borra el marcaje de esta fecha.",
        icon: "warning",
